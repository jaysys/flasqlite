...
def transaction():
    if 'username' in session:
        ...
        query = f"SELECT div, asset, to_char(qty,'9,999,999,999.999') as qty, to_char(total_krw,'9,999,999,999.9') as total_krw, asset_note, timestamp  FROM my_asset WHERE total_krw > 1000 ORDER BY timestamp desc, div, asset_note LIMIT {ROWS_PER_PAGE} OFFSET {start_index}"
